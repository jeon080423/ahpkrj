...
async def capture_screenshots():
    ...
    img1_path = os.path.join(output_dir, "screenshot1_tab2.png")
    img2_path = os.path.join(output_dir, "screenshot2_tab2.png")
    img3_path = os.path.join(output_dir, "screenshot3_tab2.png")
    ...
    async with async_playwright() as p:
        ...
        try:
            # We specifically click the exact text of the tab
            await page.get_by_text("온라인 AHP 설문지 작성 및 배포(무료)").click()
        ...
        try:
            section2 = page.get_by_text("섹션 2", exact=False).first
            await section2.scroll_into_view_if_needed()
            await page.wait_for_timeout(2000)
            await page.screenshot(path=img2_path)
        except Exception as e:
            print("Fallback scrolling to section 2")
            await page.mouse.wheel(0, 1000)
            await page.wait_for_timeout(2000)
            await page.screenshot(path=img2_path)
            
        print("Scrolling to Section 6 (CR)...")
        try:
            section6 = page.get_by_text("섹션 6", exact=False).first
            await section6.scroll_into_view_if_needed()
            await page.wait_for_timeout(2000)
            await page.screenshot(path=img3_path)
        except Exception as e:
            print("Fallback scrolling to section 6")
...
def create_docx(img1, img2, img3):
    output_dir = "AHP마스터 블로그"
    doc_path = os.path.join(output_dir, "AHP 상대비료 온라인 설문 작성 및 배포 무료_최종확정.docx")
    ...
    doc.add_heading("4. 논문 통과를 보장하는 '응답 일관성(CR) 보조 기능'", level=2)
    doc.add_paragraph("AHP 설문의 가장 큰 진입 장벽은 응답자의 논리적 일관성을 나타내는 CR(Consistency Ratio) 수치입니다. 응답자가 A가 B보다 중요하고 B가 C보다 중요하다고 응답했는데, C가 A보다 중요하다고 체크한다면 논리적 모순이 발생하여 해당 데이터는 논문에 사용할 수 없게 됩니다. 'AHP 마스터'는 관리자가 허용할 CR 한계치를 미리 지정하고, 응답자가 모순된 응답을 했을 때 실시간으로 경고를 보내거나 '스마트 보정 마법사'를 띄워 가장 모순된 문항을 스스로 고치도록 안내합니다. 이로 인해 분석 가능한 유효 응답률을 획기적으로 끌어올릴 수 있습니다.")
    
    doc.add_paragraph()
    doc.add_heading("실제 웹 화면 미리보기", level=2)
    doc.add_paragraph("아래는 실제 AHP 마스터 플랫폼에서 제공하는 온라인 설문지 작성 화면의 모습입니다.")
    ...
    if os.path.exists(img3):
        ...
        p = doc.add_paragraph("▲ 응답 일관성(CR) 한계치 및 스마트 가이드 설정 화면")
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph()
...
async def main():
    img1, img2, img3 = await capture_screenshots()
    create_docx(img1, img2, img3)
