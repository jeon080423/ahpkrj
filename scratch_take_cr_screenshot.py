...
async def capture_cr_screenshot():
    output_dir = "AHP마스터 블로그"
    os.makedirs(output_dir, exist_ok=True)
    img3_path = os.path.join(output_dir, "screenshot3_exact.png")
    ...
    async with async_playwright() as p:
        ...
        try:
            # Look for Section 6
            section6 = page.get_by_text("섹션 6: 제출 전 일관성 비율 (CR) 검증 레벨")
            await section6.scroll_into_view_if_needed()
            await page.wait_for_timeout(2000)
            await page.screenshot(path=img3_path)
        except Exception as e:
            print("Failed to find section 6:", e)
...
def create_docx(img3):
    output_dir = "AHP마스터 블로그"
    img1 = os.path.join(output_dir, "screenshot1_exact.png")
    img2 = os.path.join(output_dir, "screenshot2_exact.png")
    doc_path = os.path.join(output_dir, "AHP 상대비료 온라인 설문 작성 및 배포 무료_최종본_CR추가.docx")
...
async def main():
    img3 = await capture_cr_screenshot()
    create_docx(img3)
