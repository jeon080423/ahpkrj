...
with col_main:
    ...
    if has_admin_tab:
        with main_tab2:
            ...
            st.info("이 탭은 공인된 관리자(Admin) 전용 화면입니다. 모든 응답 데이터는 이용자 본인의 구글 계정(연동한 구글 스프레드시트)을 통해 저장되므로, 설문 배포 전에 테스트 응답을 제출하여 실제 시트에 정상적으로 데이터가 기록되는지 반드시 직접 미리 확인해야 합니다.")
