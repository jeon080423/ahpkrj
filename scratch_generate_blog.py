...
async def capture_screenshots():
    ...
    img1_path = os.path.join(output_dir, "screenshot1.png")
    img2_path = os.path.join(output_dir, "screenshot2.png")
    ...
    async with async_playwright() as p:
        ...
        # Streamlit 앱이 렌더링될 때까지 충분히 대기
        # "온라인 설문지 작성" 텍스트나 특정 h1 태그가 보일 때까지 대기
        print("Waiting for Streamlit to render...")
        await page.wait_for_timeout(8000) # 하드 타이머 추가 (Streamlit 첫 로딩 대기)
        
        # 탭2: "온라인 설문 작성 및 배포(무료)" 클릭
        print("Clicking Tab 2...")
        try:
            tabs = await page.locator('button[data-baseweb="tab"]').all()
            if len(tabs) >= 2:
                await tabs[1].click()
                await page.wait_for_timeout(5000) # 탭 전환 후 충분히 렌더링 대기
        except Exception as e:
            print("Failed to click tab:", e)
        
        # 첫 번째 캡처: 상단 안내 및 기본 설정 부분
        print("Taking first screenshot...")
        await page.screenshot(path=img1_path)
        
        # 스크롤 다운해서 두 번째 캡처: 계층구조 및 문항 작성 부분
        print("Scrolling down and taking second screenshot...")
...
def create_docx(img1, img2):
    output_dir = "AHP마스터 블로그"
    os.makedirs(output_dir, exist_ok=True)
    doc_path = os.path.join(output_dir, "AHP 상대비료 온라인 설문 작성 및 배포 무료.docx")
...
async def main():
    img1, img2 = await capture_screenshots()
