...
- **결제창 빌링키 발급**
  ...
  `billingKeyMethod` 및 `billingKeyAndPayMethod` 파라미터를 결제 수단에 따라 아래와 같이 설정해야 합니다.

  - 카드 (`requestIssueBillingKey` 함수 사용 시): `CARD`
  - 휴대폰 (`requestIssueBillingKeyAndPay` 함수 사용 시): `PHONE`
...
<Details>
  ...
  <Details.Content>
    ...
    - `range` 방식으로 입력할 경우 `from`과 `to` 모두를 입력하셔야 하며, `from`은 `to`보다 과거 시간이어야 합니다.
    ...
    <Parameter.Details>
      ...
      - phoneNumber?: string
        ...
        **구매자 연락처**
        ...
        - KG이니시스의 PC 빌링키 발급 및 결제의 경우 필수로 입력해야 합니다. (모바일인 경우에는 optional)
