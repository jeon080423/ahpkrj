...
def run():
    ...
    with tab_survey_create:
        ...
        with tab1:
            ...
            @st.dialog("📝 서식 에디터로 안내문 편집", width="large")
            def edit_yeta_desc_dialog():
                st.info("안내문 내용을 자유롭게 편집하세요. 툴바를 이용해 굵기, 색상 등을 변경할 수 있습니다.")
                new_desc = st_quill(value=st.session_state.get("edit_yeta_desc", default_survey_desc), html=True, key="quill_yeta_desc_modal_v1")
                col1, col2 = st.columns(2)
                with col1:
